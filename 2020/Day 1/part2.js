    ...
    for (let j in input_array){
        for (let k in input_array){
            if (i == j || i == k || j == k) continue;
                if (parseInt(input_array[i]) + parseInt(input_array[j]) + parseInt(input_array[k]) == 2020) {
                    console.log(parseInt(input_array[i]) * parseInt(input_array[j]) * parseInt(input_array[k]));
                    return;
                }
        }
    }
